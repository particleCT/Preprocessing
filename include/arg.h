...
#include <typeinfo>
namespace arg {
// proxy to values of command line options, need to know where to store the
// values
class Value {
public:
  virtual ~Value();
  virtual void set(const std::string &str); // convert the str to value and put
                                            // it in storage
...
class SubParser : public Value, public Parser {
  char sep;
...
template <typename T> class StreamableValue : public Value {
  T &ptr;

public:
  StreamableValue(T &t) : ptr(t) {}
  ...
  void set(const std::string &str) {
    ...
    s >> tmp;
    if (s.bad() || !s.eof())
      throw ConvError(str, typeid(T).name());
    ptr = tmp;
  }
