...
class Preprocessing { // Top level program from the pCT preprocessing task.
  ...
  void WriteBinaryFile3(bool timeStampOutput, bool energyOutput,
                        ...
                        int event_counter, double u[], float V0[], float V1[],
                        float V2[], float V3[], float T0[], float T1[],
                        float T2[], float T3[], float E1[], float E2[],
                        float E3[], float E4[], float E5[], float WetBinary[],
                        float ProjAngle[], unsigned int TimeStamp[],
                        unsigned int EventIDs[]); // float AngleNb

public:
  Preprocessing(std::string inputFileName, std::string study_name,
                std::string Outputdir, std::string WcalibFile,
                std::string TVcorrFile, int n_threads, float StgThr[5], 
		int angleBins, int analysisLevel,
                bool callUser, bool continuous_scan, float initialAngle,
                bool realTimeCal, int max_events, int max_time, int n_debug,
                int n_plot, float proj_angle,
                bool dodEEFilter, int pdstlr[5],
                std::string OsName, float Version);


  int ProcessFile(float phantomSize, std::string partType, float wedgeOffset,
                  float fileFraction, int numbTkrFPGA, int numbEdetFPGA,
                  std::string KillCh);
};
#endif
