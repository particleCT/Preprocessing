...
// V-U and T-U views.
std::vector<Tkr2D> pCT_Tracking::Tracking2D(int Idx, TkrHits &pCThits,
                                            pCTgeo const &Geometry) {
  ...
  /* Idx = 0 for V-U view
    ...
    If there are no vectors in just one T tracker
       ...
       if the position is close to a crack, place the coordinate in the middle
    of the crack
  ...
  */
  ...
  const double mxSlopeFront[2] = { 0.03, 0.09 }; // Cut on the slope of the
                                                 // front tracker vector,
                                                 // separately for V and T
  const double mxSlopeBack[2] = { 0.1, 0.15 };   // Cut on the slope of the rear
                                               // tracker vector, separately for
                                               // V and T
  const double deltaMx =
      6.0; // Cut on how far the two vectors miss each other at u=0, in mm

  // Create lists of all vectors in the front and back trackers, within the
  // specified slope cuts
  ...
  // Look at all combinations of front and rear vectors for tracks that match in
  // the center
  for (int i = 0; i < front.size(); i++) {
    for (int j = 0; j < back.size(); j++) {
      double miss = front[i].intercept - back[j].intercept;
      if (abs(miss) < deltaMx) {
        // Check whether any of the hits have already been used. Do not allow
        // hit sharing.
        if (tmp.size() > 0) { // Don't waste time if there are no tracks already
                              // (the usual situation)
          bool reject = false;
          ...
          if (reject)
            break; // This track is worse than some tracks already found that
                   // shares hits, so skip this one.
          for (int shr = 0; shr < shareList.size(); shr++) {
            tmp[shareList[shr]].Good = false; // Get rid of earlier inferior
                                              // tracks sharing hits with this
                                              // one
          }
        }
        Tkr2D tmpTk;
        ...
        tmpTk.Qh[0] = 3;
        pCThits.Lyr[0].F[Idx].at(front[i].I[0]) =
            tmp.size(); // Flags hit as used and makes a pointer from hit to
                        // track
  ...
  // With unused vectors in the back, try to make vectors in the front with an
  // unused hit and a gap, and look for a match.
  // Then repeat, using gaps in the back and unused vectors in the front.
  if (Idx == 1) { // T layers only, where the cracks are parallel to the strips.
                  // This won't work for V layers.
    for (int i = 0; i < back.size(); i++) {
      if (pCThits.Lyr[2].F[Idx].at(back[i].I[0]) < 0 &&
          pCThits.Lyr[3].F[Idx].at(back[i].I[1]) < 0) {
        int otherLyr[2] = { 1, 0 };
            ...
            for (int gap = 0; gap < 3; gap++) {
              ...
              if (abs(slope) < mxSlopeFront[Idx]) {
                ...
                if (abs(miss) < deltaMx) {
                  ...
                  tmpTk.U[lyr] = u1;
                  tmpTk.Qh[lyr] = 3;
                  pCThits.Lyr[lyr].F[Idx].at(j) =
                      tmp.size(); // Flags hit as used and makes a pointer from
                                  // hit to track
    ...
    for (int i = 0; i < front.size(); i++) {
      if (pCThits.Lyr[0].F[Idx].at(front[i].I[0]) < 0 &&
          pCThits.Lyr[1].F[Idx].at(front[i].I[1]) < 0) {
  ...
  // Now consider track candidates missing one hit in the front tracker.  Make a
  // vector from the hit that
  // is present that points back to the putative beam origin (this works a bit
  // better at LLUMC, using the
    ...
    if (pCThits.Lyr[2].F[Idx].at(back[i].I[0]) < 0 &&
      ...
      for (int lyr = 0; lyr < 2; lyr++) {
        for (int j = 0; j < pCThits.Lyr[lyr].N[Idx]; j++) {
          ...
          double slope = (t1 - Geometry.BeamVertex(Idx)[Idx]) /
                         (u1 - Geometry.BeamVertex(Idx)[2]);
          double intercept = Geometry.BeamVertex(Idx)[Idx] -
                             slope * Geometry.BeamVertex(Idx)[2];
          double miss = intercept - back[i].intercept;
          if (abs(miss) < deltaMx) {
            ...
            tmpTk.U[lyr] = u1;
            tmpTk.Qh[lyr] = 3;
            pCThits.Lyr[lyr].F[Idx].at(j) = tmp.size(); // Flags hit as used and
  ...
  // Finally, if there is an unused front vector but only one unused hit in the
  // back, try to make a track by extrapolation or interpolation
        ...
        for (int j = 0; j < pCThits.Lyr[lyr].N[Idx]; j++) {
          ...
          if (abs(slope) < mxSlopeBack[Idx]) {
            ...
            if (abs(miss) < deltaMx) {
              if (tk >= 0) {
                if (tmp[tk].Good) {
                  if (abs(tmp[tk].Miss) > abs(miss)) {
                    tmp[tk].Good = false; // This track is better than an
                                          // existing track using the same hit
                  } else {
              ...
              tmpTk.U[lyr] = u3;
              tmpTk.Qh[lyr] = 3;
              pCThits.Lyr[lyr].F[Idx].at(j) = tmp.size(); // Flags hit as used
                                                          // and makes a pointer
                                                          // from hit to track
...
pCT_Tracking::pCT_Tracking(TkrHits &pCThits,
                           pCTgeo const &Geometry) { // This class constructor
  ...
  // Count up just the good tracks in V and T.  Bad tracks were rejected because
  // they overlapped (shared hits with) superior tracks.
  int nVtkrs = 0;
  int nTtkrs = 0;
  itkV = -1;
  itkT = -1;
  ...
  if (nVtkrs == 0 || nTtkrs == 0)
    ...
    nTracks = (nVtkrs > nTtkrs) ? nVtkrs : nTtkrs; // Generally we will reject
                                                   // events if this number is
                                                   // not exactly equal to 1.
};

// Method to print out the list of tracks
void pCT_Tracking::dumpTracks(int eventNumber) {
  std::cout << "pCT_Tracking::dumpTracks: dump of the list of detected tracks "
               "for event " << eventNumber << ".  Number of tracks=" << nTracks
            << std::endl;
  std::cout << "    The first good V track is " << itkV
            << " and the first good T track is " << itkT << std::endl;
  std::cout << "    There are " << VTracks.size()
            << " tracks in the V view:" << std::endl;
  for (int i = 0; i < VTracks.size(); i++) {
    if (true) {
      //            if (VTracks[i].Good) {
      std::cout << "    Track number " << i << " Quality=" << VTracks[i].Q
                << "  Mismatch at u=0 is " << VTracks[i].Miss << " mm "
                << " good=" << VTracks[i].Good << std::endl;
      for (int lyr = 0; lyr < 4; lyr++) {
        std::cout << "          Hit on layer " << lyr
                  << "   U=" << VTracks[i].U[lyr]
                  << "   V=" << VTracks[i].X[lyr]
  ...
  std::cout << "    There are " << TTracks.size()
            << " tracks in the T view:" << std::endl;
  for (int i = 0; i < TTracks.size(); i++) {
    if (true) {
      //            if (TTracks[i].Good) {
      std::cout << "    Track number " << i << " Quality=" << TTracks[i].Q
                << "  Mismatch at u=0 is " << TTracks[i].Miss << " mm "
...
void pCT_Tracking::displayEvent(int eventNumber, TkrHits &pCThits,
                                std::string outputDir) {
  char fn[80] = "";
  char Q[5] = "ebg ";
  sprintf(fn, "%s/Event%d.gp", outputDir.c_str(), eventNumber);
  std::cout << "**** pCT_Tracking::displayEvent: Writing a single event "
               "display for event number" << eventNumber << " to file " << fn
            << std::endl;
  ...
  if (oFile != NULL) {
    ...
    fprintf(oFile, "#*** or else start up gnuplot and use the load command to "
                   "display the plot.\n");
    fprintf(oFile, "set multiplot title 'pCT Tracking Single Event Display for "
                   "Event %d' layout 2,1 columnsfirst scale 1.0,1.0\n",
            eventNumber);
    ...
    for (int lyr = 0; lyr < 4; lyr++) {
      for (int hit = 0; hit < pCThits.Lyr[lyr].N[0]; hit++) {
        ...
        if (tk < 0) { // plot unused hits
          fprintf(oFile, "%7.2f %7.2f x\n", pCThits.Lyr[lyr].U[0].at(hit),
                  pCThits.Lyr[lyr].Y[0].at(hit));
        }
      }
    }
    fprintf(oFile, "e\n");
    ...
    for (int tkr = 0; tkr < TTracks.size(); tkr++) {
      if (TTracks[tkr].Good) {
        for (int lyr = 0; lyr < 4; lyr++) {
          fprintf(oFile, "%7.2f %7.2f %d%c\n", TTracks[tkr].U[lyr],
                  TTracks[tkr].X[lyr], tkr, Q[TTracks[tkr].Qh[lyr]]);
        }
      }
    }
    for (int lyr = 0; lyr < 4; lyr++) {
      for (int hit = 0; hit < pCThits.Lyr[lyr].N[1]; hit++) {
        ...
        if (tk < 0) { // plot unused hits
          fprintf(oFile, "%7.2f %7.2f x\n", pCThits.Lyr[lyr].U[1].at(hit),
                  pCThits.Lyr[lyr].Y[1].at(hit));
        }
      }
    }
    fprintf(oFile, "e\n");
    ...
    std::cout << "pCT_Tracking::displayEvent: unable to open file for display "
                 "of event " << eventNumber << std::endl;
}
