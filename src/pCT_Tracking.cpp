...
*/
    ...
    for (int i=0; i<pCThits.Lyr[0].N[Idx]; i++) {
        for (int j=0; j<pCThits.Lyr[1].N[Idx]; j++) {
            double slope = (pCThits.Lyr[1].Y[Idx].at(j) - pCThits.Lyr[0].Y[Idx].at(i))/(pCThits.Lyr[1].U[Idx].at(j) - pCThits.Lyr[0].U[Idx].at(i));
    ...
    for (int i=0; i<pCThits.Lyr[2].N[Idx]; i++) {
        for (int j=0; j<pCThits.Lyr[3].N[Idx]; j++) {
            double slope = (pCThits.Lyr[3].Y[Idx].at(j) - pCThits.Lyr[2].Y[Idx].at(i))/(pCThits.Lyr[3].U[Idx].at(j) - pCThits.Lyr[2].U[Idx].at(i));
        ...
        for (int j=0; j<back.size(); j++) {
            ...
            if (abs(miss) < deltaMx) {
                ...
                if (tmp.size() > 0) {    // Don't waste time if there are no tracks already (the usual situation)
                    ...
                    if (reject) break;  // This track is worse than some tracks already found that shares hits, so skip this one.
    ...
    // With unused vectors in the back, try to make vectors in the front with an unused hit and a gap, and look for a match.
        ...
        if (pCThits.Lyr[2].F[Idx].at(back[i].I[0]) < 0 && pCThits.Lyr[3].F[Idx].at(back[i].I[1]) < 0) {
            ...
            for (int lyr=0; lyr<2; lyr++) {
                for (int j=0; j<pCThits.Lyr[lyr].N[Idx]; j++) {
                    ...
                    double u1 = pCThits.Lyr[lyr].U[Idx].at(j);
                    double u2; 
                    if (Idx == 1) u2 = Geometry.uT(otherLyr[lyr]); else u2 = Geometry.uV(otherLyr[lyr]);
    ...
    // Finally, if there is an unused front vector but only one unused hit in the back, try to make a track by extrapolation or interpolation
        ...
        if (pCThits.Lyr[0].F[Idx].at(front[i].I[0]) < 0 && pCThits.Lyr[1].F[Idx].at(front[i].I[1]) < 0) {
            ...
            for (int lyr=2; lyr<4; lyr++) {
                for (int j=0; j<pCThits.Lyr[lyr].N[Idx]; j++) {
                    int tk= pCThits.Lyr[lyr].F[Idx].at(j);
                    if (tk >= 0) {
                        if (tmp[tk].Good && tmp[tk].Q > 0) break;   // Skip hits already used on better tracks
                    }
                    ...
                    double u4; 
                    if (Idx == 1) u4 = Geometry.uT(otherLyr[lyr]); else u4 = Geometry.uV(otherLyr[lyr]);
...
pCT_Tracking::pCT_Tracking(TkrHits &pCThits, pCTgeo const &Geometry) {   // This class constructor finds all the tracks
    ...
    for (int lyr=0; lyr<4; lyr++) {
        if (pCThits.Lyr[lyr].N[0] > 0) nVht++;
        if (pCThits.Lyr[lyr].N[1] > 0) nTht++;
    }
    if (nVht < 3 || nTht < 3) return;    // Abort if more than one layer is missing a hit in either view

    VTracks = Tracking2D(0, pCThits, Geometry);

    if (VTracks.size() == 0) return;    // Don't waste time with the T view if there is no track in V

    TTracks = Tracking2D(1, pCThits, Geometry);

    if (TTracks.size() == 0) return;

    // Count up just the good tracks in V and T.  Bad tracks were rejected because they overlapped (shared hits with) superior tracks.
    int nVtkrs = 0;
    int nTtkrs = 0;
    itkV = -1;
    itkT = -1;
    for (int i=0; i<VTracks.size(); i++) {
        if (VTracks[i].Good) {
            nVtkrs++;
            if (itkV < 0) itkV = i;    // The first good V track
        }
    }
    for (int i=0; i<TTracks.size(); i++) {
        if (TTracks[i].Good) {
            nTtkrs++;
            if (itkT < 0) itkT = i;    // The first good T track
        }
    }
        if (nVtkrs==0 || nTtkrs==0) nTracks = 0;
    else nTracks = (nVtkrs > nTtkrs) ? nVtkrs : nTtkrs;    // Generally we will reject events if this number is not exactly equal to 1.
};

// Method to print out the list of tracks
void pCT_Tracking::dumpTracks(int eventNumber ) {
    std::cout << "pCT_Tracking::dumpTracks: dump of the list of detected tracks for event " << eventNumber << ".  Number of tracks=" << nTracks << std::endl;
    ...
    for (int i=0; i<VTracks.size(); i++) {
        if (true) {
//            if (VTracks[i].Good) {
            std::cout << "    Track number " << i << " Quality=" << VTracks[i].Q << "  Mismatch at u=0 is " << VTracks[i].Miss << " mm " << " good=" << VTracks[i].Good << std::endl;
            for (int lyr=0; lyr<4; lyr++) {
                std::cout << "          Hit on layer " << lyr << "   U=" << VTracks[i].U[lyr] << "   V=" << VTracks[i].X[lyr] << "  Q=" << VTracks[i].Qh[lyr] << std::endl;
            }
        }
    }
    ...
    for (int i=0; i<TTracks.size(); i++) {
        if (true) {
//            if (TTracks[i].Good) {
            std::cout << "    Track number " << i << " Quality=" << TTracks[i].Q << "  Mismatch at u=0 is " << TTracks[i].Miss << " mm " << " good=" << TTracks[i].Good << std::endl;
            for (int lyr=0; lyr<4; lyr++) {
                std::cout << "          Hit on layer " << lyr << "   U=" << TTracks[i].U[lyr] << "   T=" << TTracks[i].X[lyr] << "  Q=" << TTracks[i].Qh[lyr] << std::endl;
            }
        }
    }
};
...
void pCT_Tracking::displayEvent(int eventNumber, TkrHits &pCThits, std::string outputDir) {
    ...
    std::cout << "**** pCT_Tracking::displayEvent: Writing a single event display for event number" << eventNumber << " to file " << fn << std::endl;
    ...
    if (oFile != NULL) {
        ...
        fprintf(oFile,"set multiplot title 'pCT Tracking Single Event Display for Event %d' layout 2,1 columnsfirst scale 1.0,1.0\n", eventNumber);
        ...
        for (int tkr=0; tkr<VTracks.size(); tkr++) {
            if (VTracks[tkr].Good) {
                for (int lyr=0; lyr<4; lyr++) {
                    fprintf(oFile,"%7.2f %7.2f %d%c\n", VTracks[tkr].U[lyr], VTracks[tkr].X[lyr], tkr, Q[VTracks[tkr].Qh[lyr]]);  // plot hits on tracks
                }
            }
        }
        for (int lyr=0; lyr<4; lyr++) {
            for (int hit=0; hit<pCThits.Lyr[lyr].N[0]; hit++) {
                ...
                if (tk >= 0) {if (!VTracks[tk].Good) tk = -1;}
        ...
        for (int lyr=0; lyr<4; lyr++) {
            for (int hit=0; hit<pCThits.Lyr[lyr].N[1]; hit++) {
                ...
                if (tk >= 0) {if (!TTracks[tk].Good) tk = -1;}
                if (tk < 0) {                // plot unused hits
    ...
    else std::cout << "pCT_Tracking::displayEvent: unable to open file for display of event " << eventNumber << std::endl;
}
