...
bool Option::take_value() { return store_ptr; }

bool Option::need_value() { return store_ptr && !store_optional; }

int Option::get_key() { return key; }

const string &Option::get_name() { return name; }
...
void Option::process() {
  if (store_ptr) {
    if (!store_optional)
      throw OptError(name, "missing value");
    store_ptr->set(store_str);
  }
  if (set_var) {
    if (set_once && set_init != *set_var)
      throw OptError(name, "can not re-set");
    *set_var = set_value;
  }
  if (call_func) {
    if (!(*call_func)(key, "", call_data))
      throw OptError(name, "callback error");
  }
}

void Option::process(const string &str) {
  if (!store_ptr)
    throw OptError(name, "unwanted value '" + str + "'");
  store_ptr->set(str);
  if (set_var) {
    if (set_once && set_init != *set_var)
      throw OptError(name, "can not re-set");
  ...
  if (call_func) {
    if (!(*call_func)(key, str, call_data))
      throw OptError(name, "callback error");
  }
}
...
void Parser::set_header(const std::string &text) { header_text = text; }
...
string Error::get_msg() { return msg; }
