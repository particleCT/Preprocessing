...
void TkrHits::dumpHits(int eventNumber) {
  std::cout << "List of the coordinates in V and T layers for event "
            << eventNumber << ":" << std::endl;
  for (int lyr = 0; lyr < 4; lyr++) {
    std::cout << "    Layer number " << lyr << " has " << Lyr[lyr].N[0]
              << " V hits and " << Lyr[lyr].N[1] << " T hits\n";
    if (Lyr[lyr].N[0] <= 0)
      std::cout << "        No V coordinates found." << std::endl;
    else {
    ...
    if (Lyr[lyr].N[1] <= 0)
      std::cout << "        No T coordinates found." << std::endl;
    else {
    ...
    if (fpga.num_chips != 0) {
      ...
      for (int ichip = 0; ichip < fpga.num_chips; ichip++) {
        ...
        if (chip_ID < 0 || chip_ID > 11) {
          std::cout << "TkrHits **********Data Error********* Chip ID "
                    << chip_ID << " is out of range!!\n";
          break;
        }
        if (print)
          std::cout << "  Chip " << chip_ID
                    << " number of clusters=" << chip.num_clusts << "\n";
        if (chip.num_clusts > 10) {
          std::cout
              << "TkrHits ************Data Error********* Number of clusters = "
              << chip.num_clusts << "!!\n";
          break;
        }
        for (int icluster = chip.num_clusts - 1; icluster >= 0; icluster--) {
          pCTraw::TrackerFPGA::TrackerChip::TrackerCluster cluster =
              chip.cluster[icluster];
          ...
          if (ichip != 0) {
            if (print)
              std::cout << "      Cluster " << icluster
                        << "  First strip=" << cluster.first
                        << "  Number strips=" << cluster.length << "\n";
            if (chip_ID != 0 && chip_ID != 6) {
              pCTraw::TrackerFPGA::TrackerChip::TrackerCluster prevClus =
                  prevChip.cluster[0];
              int lastStrip = cluster.first + cluster.length;
              int firstStrip = prevClus.first;
              int prevChip_ID = prevChip.address;
              if (print)
                std::cout << "        Last strip=" << lastStrip
                          << " first strip previous chip=" << firstStrip
                          << " Chip=" << chip_ID
                          << " Previous chip=" << prevChip_ID << "\n";
              if (lastStrip == 63 && prevChip_ID == chip_ID - 1 &&
                  firstStrip == 0 && cluster.length != 63) {
                if (idx0 < 0 || cl1[idx0] != 0 || chip_ID != clcp[idx0] + 1) {
                  std::cout << "**TkrHits: OOPS!  Logic screwed up, idx0="
                            << idx0 << " cl1=" << cl1[idx0] << " ID=" << chip_ID
                            << " clcp=" << clcp[idx0] << "\n";
                  std::cout << "**FPGA=" << iFPGA << " chip_ID=" << chip_ID
                            << " ichip=" << ichip << " icluster=" << icluster
                            << "\n";
                  pCTEvent.dumpEvt();
                } else {
                  ...
                  merge = true;
                  if (print)
                    std::cout << "          Merging FPGA " << iFPGA << " chip "
                              << chip_ID << " cluster " << cluster.first << " "
                              << cluster.length << " idx0=" << idx0 << "\n";
                }
              }
            }
          }
      ...
      for (int icl = 0; icl < ncl; icl++) {
        if (print)
          std::cout << "          Merged cluster " << icl << " 1st strip "
                    << cl1[icl] << " length " << cln[icl] << "  chip "
                    << clcp[icl] << "\n";
        ...
        if (iFPGA < 4) {
          ...
          Lyr[Geometry.Lyr(iFPGA)].U[0].push_back(
              Geometry.uV(Geometry.Lyr(iFPGA)));
          ...
          Lyr[Geometry.Lyr(iFPGA)].U[1].push_back(
              Geometry.uT(Geometry.Lyr(iFPGA)));
